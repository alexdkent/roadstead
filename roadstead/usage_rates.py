...
_RATES_BY_CLASS: dict[str, tuple[float, float]] = {
    ...
    # 2026-08-22 cutover: tier3 moved off Laguna S 2.1 INT4 onto DeepSeek-V4-Flash-0731
    # (two-node vLLM TP=2, anvil + anvil2). ANCHOR CHANGE, not just a number change:
    # every other row here is a PROXY ("the closest hostable open model, mid-of-market").
    # This row stops being a proxy — V4 Flash is served by its own vendor at a published
    # rate, so tier3 is now the only EXACT-model anchor in the table.
    #
    # THE SAVINGS FIGURE STEPPED DOWN ON CUTOVER DAY. Output fell 0.60 → 0.28, so the
    # cloud-equivalent cost of a tier3 completion dropped by more than half on the output
    # leg (a 1M-in/1M-out call: $0.75 → $0.42). Nothing is broken; the fleet just stops
    # being credited for renting a 235B when it is running a 284B-total/13B-active model
    # that is cheap to rent.
    #
    # IT REPRICES HISTORY RETROACTIVELY. Unlike the 2026-08-19 tier2 split, this cutover
    # does NOT change the endpoint CLASS — pre- and post-cutover rows in
    # `proxy_completions` both carry `thinker`, so this in-place edit prices every Laguna
    # row that ever ran at DeepSeek's rate too. Accepted deliberately: minting a new class
    # here would mean changing `endpoint_class` in models.yaml, i.e. changing ROUTING and
    # DRR accounting to fix a reporting artifact — and an endpoint class that is also an
    # alias is the `endpoint-class-alias-collision` shape.
    "thinker":   (0.14, 0.28),   # DeepSeek-V4-Flash-0731 (DSpark) → its OWN published API rate
