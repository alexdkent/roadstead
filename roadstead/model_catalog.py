...
class EndpointEntry:
    ...
    #: Conservative decode rate, output tokens/sec, for the SLOW TAIL of real
    #: contended traffic — a low percentile of observed tok/s, never a
    #: single-stream idle benchmark (that number runs ~2x optimistic and
    #: produces a floor too low to catch the failure it exists to prevent).
    #: Seeds TimeoutModel's per-class decode-rate floor
    #: (timeout_model.decode_rate_floor_ms) — a fleet MEASUREMENT, never a
    #: value this repo invents. Absent (0.0, the default) means no floor for
    #: the class: ``build_class_decode_rates`` filters it out like every
    #: other ``> 0`` catalog field.
...
def _coerce_endpoint(name: str, raw: dict[str, Any]) -> EndpointEntry:
    ...
    return EndpointEntry(
        ...
        decode_tok_s=float(raw.get("decode_tok_s", 0) or 0),
...
def build_class_decode_rates(cat: Catalog | None = None) -> dict[str, float]:
    """endpoint class → conservative decode rate (output tokens/sec), slow-tail.
    ...
    no ``decode_tok_s`` (or 0), so a model nobody profiled gets no entry —
    which is what makes "absent rate ⇒ no floor" the default rather than a
    special case this function has to remember."""
    return _class_map(cat or load_catalog(), "decode_tok_s")
