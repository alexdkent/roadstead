...
`ProxyService` forwards ~30 HTTP methods straight to `ProxyHttpHandlers`. The
delegation is mechanical, so nothing enforces it: change a handler's signature
and the delegator still *compiles*, still forwards, and fails at runtime on the
one route nobody exercised.
...
HANDLERS = _ROOT / "roadstead" / "http_handlers.py"
...
def _delegations() -> list[tuple[str, str]]:
    """(service method, handler method) for every `self._http.X(...)` call."""
    out = []
...
@pytest.mark.parametrize("svc_name,handler_name", _delegations(),
                         ids=lambda v: v if isinstance(v, str) else str(v))
def test_delegator_signature_matches_its_handler(svc_name, handler_name):
    handlers = _methods(HANDLERS, "ProxyHttpHandlers")
    assert handler_name in handlers, (
        f"ProxyService.{svc_name} forwards to _http.{handler_name}, which does "
        f"not exist on ProxyHttpHandlers")
