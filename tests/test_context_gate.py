...
async def test_shadow_counts_but_admits():
    ...
    await svc.startup()
    try:
        # chat = 32768/slot; 50K tokens ≈ 200K chars blows it.
        ...
        tally = svc._context_overflows["chat"]
        assert tally["count"] == 1
        assert tally["callers"] == {"a": 1}
        assert tally["max_est_in"] >= 49_000
        status = json.loads((await svc.handle_status(_Req())).body)
        assert "chat" in status["reliability"]["context_overflows_shadow"]
    finally:
        await svc.shutdown()
...
async def test_enforce_422_with_chunker_marker_and_code():
    ...
    resp = await svc.handle_submit(_body("chat", 200_000), _Req())
...
async def test_enforce_max_tokens_counts_toward_limit():
    svc = ProxyService(ProxyConfig())
    svc._flags.set_many({"context_gate_enforce": True})
    # chat: 32768/slot. est_in ≈ 30000 fits alone; +4000 max_tokens overflows.
    resp = await svc.handle_submit(
        _body("chat", 120_000, max_tokens=4000), _Req())
    assert resp.status_code == 422
...
async def test_boundaries_thinker_vs_companion():
    ...
    await svc.startup()
    try:
        # ~50K tokens: over companion's 32768/slot, comfortably inside the
        # thinker's 131072 — the gate is PER-ENDPOINT.
        big = 200_000
        resp = await svc.handle_submit(_body("qwen-composer", big), _Req())
        assert resp.status_code == 422
        resp = await asyncio.wait_for(
            svc.handle_submit(_body("llama-thinker", big, timeout_s=10.0), _Req()),
            timeout=10.0)
...
async def test_non_chat_and_unknown_context_skip_the_gate():
    ...
    try:
        ...
        svc._config.endpoints["chat"].context_per_slot = 0
        resp = await asyncio.wait_for(
            svc.handle_submit(_body("chat", 200_000, timeout_s=10.0), _Req()),
            timeout=10.0)
        assert resp.status_code == 200
        assert "chat" not in svc._context_overflows
    finally:
        await svc.shutdown()
