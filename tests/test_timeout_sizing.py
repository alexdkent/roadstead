...
D2 — ``_IN_EDGES`` collapsed 17K..700K into ONE cell
    ...
    of exactly 180000.0 (the tier3 floor) for 123K-token prompts.
...
def _model(**kw) -> TimeoutModel:
    return TimeoutModel(margin=1.5, min_samples=30, floors={"tier3": 180.0}, **kw)


def _fill(m: TimeoutModel, *, n: int, in_tokens: int, out_tokens: int,
          latency_ms: float, pri: int = 3) -> None:
    for _ in range(n):
        m.record("tier3", pri, in_tokens, out_tokens, latency_ms, "ok", _NOW)
...
def test_stretch_at_the_largest_served_context_is_sane():
    ...
    lift the 180s tier3 floor into that territory, and no further than the
...
def test_long_context_resolves_to_a_long_context_cell_not_the_17k_catch_all():
    ...
    timeout events showed (``recommended_ms`` == 180000.0, the tier3 floor,
    ...
    long_ctx = m.advise("tier3", 3, 120_000, 400)
    short_ctx = m.advise("tier3", 3, 22_000, 400)
...
def test_missing_output_budget_is_unknown_not_the_shortest_output_bucket():
    ...
    unknown = m.advise("tier3", 3, 4_000, 0)
    tiny = m.advise("tier3", 3, 4_000, 60)
...
def test_advice_never_shrinks_as_the_prompt_grows():
    ...
    ~12.5 hours of tier3 time on the output axis."""
    m = _model()
    ...
    for est_in in (500, 2_000, 8_000, 22_000, 40_000, 80_000, 120_000,
        ...
        rec = m.advise("tier3", 3, est_in, 400)["recommended_ms"]
...
def test_advice_is_monotonic_across_the_full_in_x_out_grid():
    ...
    grid = {
        (i, o): m.advise("tier3", 3, i, o)["recommended_ms"]
...
def test_the_in_axis_lift_is_reported():
    ...
    big = m.advise("tier3", 3, 400_000, 400)
