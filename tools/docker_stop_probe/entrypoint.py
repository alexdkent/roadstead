...
def main() -> None:
    """🚨 Everything lives in here because this module STARTS A SERVER and never
    returns. At import time it must do nothing — `tests/test_tools_are_importable.py`
    imports every script under `tools/`, and it only misses this one because its
    glob is non-recursive. That is luck, not a design; an import-safe module
    does not depend on nobody widening a glob."""
