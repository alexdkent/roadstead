# Changelog archive — the pre-0.1.0 engineering narrative

This is the working changelog Roadstead kept while it was being extracted from a private monorepo
into a project of its own: **2026-08-31 to 2026-09-05**, sixty-five entries that lived under a
single `## Unreleased` heading and were moved here verbatim when 0.1.0 was cut.

It is kept because those entries are the reasoning rather than the summary. Most of them record what
was measured, what was believed before the measurement, and what was rejected and why — at a length
a release note cannot carry. `CHANGELOG.md`'s `## 0.1.0` section links in here for the entries whose
long version is worth reading.

**From 0.1.0 on, `CHANGELOG.md` is the release summary**: grouped, short, one section per release.
Nothing is added to this file; it is a closed record of how the project reached its first tag.

⚠️ **Read these in their own tense.** Each was written as its work landed, so several describe as
open something that later closed — the scrub sections in particular are a running account of a job
in progress, and the one that says every commit SHA changed on 2026-09-01 means it: any SHA cited
by an entry written before that date is dead.
...
- **🚨 And the personal identifiers inside those fixtures, which the scrub plan had ruled out.** Its
  ...
  straggler sweep in the scrub plan (archived privately) grew a second pattern, because a grep for the
