...
is deliberately NOT in this tree — it names the real hosts still in the history and the exact
greps that find them, so publishing it would hand a reader the search. It is kept in the origin
monorepo; the part a contributor needs is inlined into `CONTRIBUTING.md`.
