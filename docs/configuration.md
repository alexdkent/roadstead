...
| `ROADSTEAD_REQUIRE_API_KEY` | 🔒 | opt-in; **off** | Refuse any request **to the inference and management doors** presenting no credential. 🚨 It does NOT close the open analytics family (§3.12 of `docs/api.md` — `/v1/status`, `/v1/recent`, `/v1/usage`, `/metrics`, `/v1/models` and the rest): that family is open by design and is enforced at a different layer, so setting this flag does not make the caller roster or the per-caller token volumes private. Deploy-time network policy is what closes those. The right setting for a proxy reachable from anything wider than one host; wrong for the laptop case, so it is a decision, not a default. Set with no keys configured and every request is refused — startup warns. |
...
| variable | 🔒 | type / default | what it does |
|---|---|---|---|
| `ROADSTEAD_SOURCE_SHA` | 🔒 | git SHA; `unknown` | The source commit this process was built from, reported as `build_sha` by `GET /v1/status`. Set it from the SAME deploy-time value that stamps the image's `org.roadstead.source-sha` label, so the endpoint and the label cannot disagree. Deliberately not an image tag: a tag like `roadstead:fleet` is rewritten in place on every deploy and names a stream rather than a commit. `unknown` means the deploy did not supply it. |
...
| `ROADSTEAD_PROXY_REASONING_LOOP_BREAK` | 🔓 | kill-switch; **on** | Watch the **reasoning** channel of a live stream for a long-period repetition cycle and stop a run that has provably stopped making progress. Distinct from the degeneration guard above in channel, shape and timing: that one judges reassembled content after the fact, this one judges reasoning while it is still being generated — the only point at which the wasted time is still reclaimable. Inert unless the endpoint declares all four `reasoning_loop_*` thresholds, so "on" costs nothing where nothing is configured. |
| `ROADSTEAD_PROXY_REASONING_LOOP_SHADOW` | 🔓 | opt-in; **on** | Demote that guard to detect-and-count — never interrupts the stream. **Defaults ON**, unlike the degeneration guard's shadow: that guard was calibrated on 12,899 real completions, this one on 14 traces from a single prompt shape on one model. Measure on live traffic first; set to `0` to let it act. |
| `ROADSTEAD_PROXY_REASONING_LOOP_ANSWER_NOW` | 🔓 | opt-in; **off** | After an interrupted reasoning loop, make ONE more bounded, non-thinking call that writes the answer from the pre-loop notes and stream it to the caller, instead of returning the loop-break error. Opt-in because it issues a **second backend call**. Inert while `..._LOOP_SHADOW` is on (nothing is interrupted, so there is nothing to rescue). Chosen over vLLM's forced `reasoning_end_str`, which on the lane this was built for produced either zero content or 300 KB of scratchpad relabelled as the answer — a forced end stops the parser, not the model. |
| `ROADSTEAD_PROXY_STRUCTURED_BLANK_RUN_ABORT` | 🔓 | kill-switch; **on** | Watch the **content** channel of a structured (JSON-schema / `json_object`) call for a run of consecutive blank/whitespace chars long enough to only be the whitespace-loop degeneration, and stop it before `max_tokens` does — a sibling of the reasoning loop-break guard above in channel (content, not reasoning) and mechanism (a trailing-run length, not a distinct-gram ratio). Streaming aborts outright; sync salvages grammar-legal trailing whitespace or re-dispatches once. Inert unless the endpoint declares a positive `structured_blank_run_abort_chars`, so "on" costs nothing where nothing is configured. See `docs/api.md` §3.15. |
